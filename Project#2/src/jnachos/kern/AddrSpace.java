...
public class AddrSpace {
	...
	//public static Semaphore mutex = new Semaphore("MUTEX", 1);
	...
	public AddrSpace(OpenFile executable) {
		...
		for (int i = 0; i < mNumPages; i++) {
			...
			//mPageTable[i].SwapSpacePosition = index;
			...
			if ((i * Machine.PageSize) < (noffH.code.size + noffH.initData.size)) {
				...
				//JNachos.mutex.P();
				//read the code into mSwapspace
				System.out.println("Page " + JNachos.SwapSpaceindex );
				//index translation entry 里 每次读  page size
				//mPageTable[i].SwapSpacePosition = JNachos.SwapSpaceindex;
				//JNachos.mSwapSpace.writeAt(from, numBytes, position);
				//
				JNachos.mSwapSpace.writeAt(bytes, bytes.length, JNachos.SwapSpaceindex * Machine.PageSize);
				//JNachos.SwapSpaceindex++;
				//index++;
				//JNachos.mutex.V();
				
				// When a process starts we only put its first page in to RAM.
				if(i==0) {
					...
					//NachosProcess process;
					Entry.setProcess(JNachos.getCurrentProcess());					
					Entry.setVirtualPageNumbaer(i); 
					
					JNachos.queue.offer(Entry);			
				}				
			}
			System.out.println("Swap Space: "+ JNachos.SwapSpaceindex +" page");
			mPageTable[i].swapSpacePlace = JNachos.SwapSpaceindex;
			JNachos.SwapSpaceindex++;
		}
	}
