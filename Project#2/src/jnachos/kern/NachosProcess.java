...
public class NachosProcess implements Runnable {
	...
	public NachosProcess(String pProcessName) {
		...
		//When create a new process, give it a unique id and print it
		mid = ProcessId.giveId();
	...
	public void finish() {
		...
		TranslationEntry[] pageTable=MMU.mPageTable;
		for (entry pventry: JNachos.queue) {
			int pageNum=pventry.getVirtualPageNum();
			if(pventry.getProcess().getId()==JNachos.getCurrentProcess().getId())
				Arrays.fill(Machine.mMainMemory, pageTable[pageNum].physicalPage * Machine.PageSize,
					(pageTable[pageNum].physicalPage + 1) * Machine.PageSize, (byte) 0);
		}
	...
	public void saveReg(int num, int value) {
		mUserRegisters[num]=value;
	}
	
	public int readReg(int num) {
		return mUserRegisters[num];
	}
