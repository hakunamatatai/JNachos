...
public class NachosProcess implements Runnable {
	...
	/**
	* Define the process's PID
	*/
	private static Integer INIT_PID = 529;
	private Integer mPID;
	
	//Process queue
	public Queue<Buffer> ProcessQueue = new LinkedList<Buffer>();
	...
	public NachosProcess(String pProcessName) {
		...
		System.out.println("process name " + mName);
		...
		//process queue
		//ProcessQueue = new LinkedList<Buffer>();
		
		//get the set PID form ProcessId class
		//PID = ProcessId.getID();
		//ProcessId.IDList.put(PID, this);
		//ProcessId.waitinglist.put(PID, null);
		//System.out.println("The   " + pProcessName + " process id is : " + PID);
		
		mPID = INIT_PID + 1;
		INIT_PID++;

		//Child_Parent_Map[mPID] = JNachos.getCurrentProcess().getPID();
		JNachos.ProcessTable.put(mPID, this);
		
		//ProcessNameTable
		JNachos.ProcessNameTable.put(mName, this);
	 ...
	 * 
	 * @return int pid for this thread.
	 */
	//public int getID() {
	//	return PID;
	//}
